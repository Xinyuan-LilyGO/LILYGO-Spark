...
const BurnerModal: React.FC<BurnerModalProps> = ({ file, onClose }) => {
    ...
    const handleBurn = async () => {
        ...
        if (toolStrategy === 'native') {
            ...
            // esptool-js needs the Web Serial `SerialPort` object, which this modal
            // never has: handleSelectDeviceClick discards requestPort()'s return
            // value and lets Electron's picker populate the list instead, and an
            // Electron portId can't be mapped back to a Web Serial port. The main
            // Burner tab keeps that object, so it supports this strategy; here we
            // refuse explicitly rather than silently doing nothing.
            xtermRef.current?.writeln('Error: esptool-js is not available for downloaded firmware. Please select "esptool (Native)".');
            setStatus('error');
        }
    };
